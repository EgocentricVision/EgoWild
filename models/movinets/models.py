# Code credits to https://github.com/Atze00/MoViNet-pytorch
...
class CausalModule(nn.Module):
    ...
    def reset_activation(self, source_type: str = None) -> None:
        if source_type is None:
            self.activation = dict()
        else:
            self.activation.pop(source_type, None)


class TemporalCGAvgPool3D(CausalModule):
    def __init__(self) -> None:
        super().__init__()
        self.n_cumulated_values = dict()
        self.register_forward_hook(self._detach_activation)

    def forward(self, x: Tensor, source_type: str, update_activation: bool = True) -> Tensor:
        input_shape = x.shape
        device = x.device
        ...
        divisor = (torch.arange(1, input_shape[2] + 1,
                   ...
                   .expand(x.shape))
        if self.activation.get(source_type, None) is None and update_activation:
            self.activation[source_type] = cumulative_sum[:, :, -1:].clone()
        else:
            if self.activation.get(source_type, None) is not None:
                cumulative_sum += self.activation[source_type]
            if update_activation:
                self.activation[source_type] = cumulative_sum[:, :, -1:].clone()

                if self.n_cumulated_values.get(source_type, None) is None:
                    self.n_cumulated_values[source_type] = 0

                self.n_cumulated_values[source_type] += input_shape[2]

        if self.n_cumulated_values.get(source_type, None) is None and update_activation:
            self.n_cumulated_values[source_type] = 0
            self.n_cumulated_values[source_type] += input_shape[2]

        if self.n_cumulated_values.get(source_type, None) is None:
            x = cumulative_sum / divisor
        else:
            x = cumulative_sum / (self.n_cumulated_values[source_type] + divisor)
        return x
    ...
    def _detach_activation(module: CausalModule,
                           input: Tensor,
                           output: Tensor) -> None:
        for k in module.activation.keys():
            module.activation[k].detach_()

    def reset_counter(self, source_type: str = None) -> None:
        if source_type is None:
            self.n_cumulated_values = dict()
        else:
            self.n_cumulated_values[source_type] = 0
    ...
    def reset_activation(self, source_type: str = None) -> None:
        super().reset_activation(source_type)
        self.reset_counter(source_type)
...
class ConvBlock3D(CausalModule):
    ...
    def _forward(self, x: Tensor, source_type: str, update_activation: bool = True) -> Tensor:
        device = x.device
        if self.dim_pad > 0 and self.conv_2 is None and self.causal is True:
            x = self._cat_stream_buffer(x, device, source_type, update_activation)
    ...
    def forward(self, x: Tensor, source_type: str, update_activation: bool = True) -> Tensor:
        ...
        x = self._forward(x, source_type, update_activation)
        return x

    def _cat_stream_buffer(self, x: Tensor, device: torch.device, source_type: str,
                           update_activation: bool = True) -> Tensor:
        if self.activation.get(source_type, None) is None:
            self._setup_activation(x.shape, source_type)
        x = torch.cat((self.activation[source_type].to(device), x), 2)
        if update_activation:
            self._save_in_activation(x, source_type)
        return x

    def _save_in_activation(self, x: Tensor, source_type: str) -> None:
        assert self.dim_pad > 0
        self.activation[source_type] = x[:, :, -self.dim_pad:, ...].clone().detach()

    def _setup_activation(self, input_shape: Tuple[float, ...], source_type: str) -> None:
        assert self.dim_pad > 0
        self.activation[source_type] = torch.zeros(*input_shape[:2],  # type: ignore
                                                   self.dim_pad,
                                                   *input_shape[3:])
...
class SqueezeExcitation(nn.Module):
    ...
    def _scale(self, input: Tensor, source_type: str, update_activation: bool = True) -> Tensor:
        if self.causal:
            x_space = torch.mean(input, dim=[3, 4], keepdim=True)
            scale = self.temporal_cumulative_GAvg3D(x_space, source_type, update_activation)
            scale = torch.cat((scale, x_space), dim=1)
        else:
            scale = F.adaptive_avg_pool3d(input, 1)

        scale = self.fc1(scale, source_type, update_activation)
        scale = self.activation_1(scale)
        scale = self.fc2(scale, source_type, update_activation)
        return self.activation_2(scale)

    def forward(self, input: Tensor, source_type: str, update_activation: bool = True) -> Tensor:
        scale = self._scale(input, source_type, update_activation)
        return scale * input
    ...
    def forward(self, input: Tensor, source_type: str, update_activation: bool = True) -> Tensor:
        ...
        if self.res is not None:
            for layer in self.res:
                if type(layer) is ConvBlock3D:
                    x = layer(x, source_type, update_activation)
                else:
                    x = layer(x)

        residual = x
        if self.expand is not None:
            x = self.expand(input, source_type, update_activation)
        else:
            x = input
        x = self.deep(x, source_type, update_activation)
        x = self.se(x, source_type, update_activation)
        x = self.project(x, source_type, update_activation)
...
class MoViNet(nn.Module):
    ...
    def avg(self, x: Tensor, source_type: str, update_activation: bool = True) -> Tensor:
        if self.causal:
            avg = F.adaptive_avg_pool3d(x, (x.shape[2], 1, 1))
            avg = self.cgap(avg, source_type, update_activation)[:, :, -1:]
        else:
    ...
    def _forward_impl(self, x: Tensor, source_type: str, update_activation: bool = True) \
            -> tuple[Any, dict[str, Union[Tensor, Any]]]:
        spatial_features = None
        x = self.conv1(x, source_type, update_activation)
        for layer in self.blocks:
            if type(layer) is ConvBlock3D or type(layer) is BasicBneck:
                x = layer(x, source_type, update_activation)
            else:
                x = layer(x, source_type, update_activation)
        x = self.conv7(x, source_type, update_activation)
        feat = F.adaptive_avg_pool3d(x, (x.shape[2], 1, 1)).squeeze(-1).squeeze(-1)

        x = self.avg(x, source_type, update_activation)

        for i_l, layer in enumerate(self.classifier):
            if type(layer) is ConvBlock3D:
                x = layer(x, source_type, update_activation)
            else:
                x = layer(x)
        ...
        x = x.flatten(1)
        return x, {"features": feat, "spatial_features": spatial_features}

    def forward(self, x: Tensor, source_type: str = "source", update_activation: bool = True) \
            -> tuple[Any, dict[str, Union[Tensor, Any]]]:

        return self._forward_impl(x, source_type, update_activation)

    @staticmethod
    def _clean_activation_buffers(m, source_type: str = None):
        if issubclass(type(m), CausalModule):
            m.reset_activation(source_type)

    @staticmethod
    def _clean_avg_counter(m, source_type: str = None):
        if isinstance(m, TemporalCGAvgPool3D):
            m.reset_activation(source_type=source_type)

    def clean_activation_buffers(self, source_type: str = None) -> None:
        self.apply(lambda mod: self._clean_activation_buffers(mod, source_type))

    def clean_avg_counter(self, source_type: str = None) -> None:
        self.apply(lambda mod: self._clean_avg_counter(mod, source_type))
